...
# Timeout Fix (It was defaulting to 300 seconds and causing a Glitch every 5 minutes)
...
print("ENV VARIABLE TEST AS FOLLOWS:" + os.environ.get('___SENSITIVE_AZURE_OPENAI_API_KEY'))
...
def connectAvatar() -> Response:
    ...
    try:
        logging.debug(f"Starting avatar connection setup for client {client_id}")
        if speech_private_endpoint:
        ...
        logging.debug(f"Creating SpeechSynthesizer for client {client_id}")
        ...
        logging.debug(f"ICE token object: {json.dumps(ice_token_obj)}")
            ...
            'synthesis': {
                'video': {
                    ...
                    'format':{
                        'crop':{
                            'topLeft':{
                                'x': 475 if video_crop.lower() == 'true' else 0,
                                'y': 0
                            },
                            'bottomRight':{
                                'x': 1475 if video_crop.lower() == 'true' else 1920,
                                'y': 800
                            }
                        },
                        'bitrate': 1000000
                    ...
                    'talkingAvatar': {
                        ...
                        'style': avatar_style,
                        # #00B140FF - chroma green, #009D57FF - television green, #0C8918FF - digital TV green   original crop: TL: x:600,y:0 - BR: 1320,1080
                        'background': {
        ...
        logging.debug(f"Avatar config: {json.dumps(avatar_config)}")      
...
# Refresh the speech token every 4 minutes
def refreshSpeechToken() -> None:
    ...
    while True:
        ...
        logging.debug("Refreshed Speech Token")
        time.sleep(60 * 4)
...
# Speak the given text.
def speakText(text: str, voice: str, speaker_profile_id: str, ending_silence_ms: int, client_id: uuid.UUID) -> str:
    # Escape the input text to prevent SSML injection, but not the phoneme markup
    escaped_text = html.escape(text)
    # Check if "CERAWeek" is in the text and adjust pronunciation with raw SSML markup
    if "CERAWeek" in text:
        modified_text = re.sub(
            r'CERAWeek',
            '<phoneme alphabet="ipa" ph="ˈsɛrə wiːk">CERAWeek</phoneme>',
            escaped_text,
            flags=re.IGNORECASE
        )
    else:
        modified_text = escaped_text

    # Construct SSML without escaping the phoneme tags
    ssml = f"""<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' xmlns:mstts='http://www.w3.org/2001/mstts' xml:lang='en-US'>
                 <voice name='{voice}'>
                     <mstts:ttsembedding speakerProfileId='{speaker_profile_id}'>
                         <mstts:leadingsilence-exact value='0'/>
...
speechTokenRefereshThread = threading.Thread(target=lambda: logging.debug("Speech token refresh thread started") or refreshSpeechToken())
