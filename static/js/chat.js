...
var welcomeMessageSpoken = false;
    ...
    .then(response => {
        if (response.ok) {
            ...
            response.text().then(text => {
                ...
                speechRecognitionConfig.setProperty("SpeechContext-PhraseDetection.TrailingSilenceTimeout", "1500")
        ...
        if (response.ok) {
            response.text().then(text => {
                ...
                .then(() => {
					...
					//only send Greeting on initial load
					if (welcomeMessageSpoken == false)
					{
                    				sendGreetingMessage();
...
function sendGreetingMessage() {
	...
	//const greetingText = `With a Period at the end: <phoneme alphabet="ipa" ph="ˈsɛrə wiːk">CERAWeek</phoneme>. With an Exclamation Point at the end: <phoneme alphabet="ipa" ph="ˈsɛrə wiːk">CERAWeek</phoneme>! With a Semicolon at the end: <phoneme alphabet="ipa" ph="ˈsɛrə wiːk">CERAWeek</phoneme>; With a Question Mark at the end: <phoneme alphabet="ipa" ph="ˈsɛrə wiːk">CERAWeek</phoneme>?`;
...
function handleGlobalMouseDown(event) {
    // Define excluded elements (function buttons and chat window)
    const excludedSelectors = [
        '#startSession',       // Start Session button
        '#stopSession',        // Stop Session button
        '#stopSpeaking',       // Stop Speaking button
        '#microphone',         // Microphone button
        '#clearChatHistory',   // Clear Chat History button (if exists)
    ...
    while (target && target !== document) {
        if (excludedSelectors.some(selector => target.matches(selector))) {
            isExcluded = true;
            break;
        }
    ...
    // If click is not on an excluded element, trigger window.microphone
    if (!isExcluded) {
        window.microphone();
    }
}
...
// Function to replace specific phrases with "CERAWeek"
function normalizeUserQuery(query) {
    const replacements = ["Sarah Week", "Sara Week", "Sarawak"];
    let normalizedQuery = query;
    for (const phrase of replacements) {
        normalizedQuery = normalizedQuery.replace(new RegExp(phrase, 'gi'), "CERAWeek");
    }
    return normalizedQuery;
}

// Updated window.microphone function
window.microphone = () => {
    ...
    speechRecognizer.recognized = async (s, e) => {
        if (e.result.reason === SpeechSDK.ResultReason.RecognizedSpeech) {
            ...
            // Normalize the user query to replace "Sarah Week", "Sara Week", or "Sarawak" with "CERAWeek"
            let normalizedQuery = normalizeUserQuery(userQuery);
            ...
            // Display the normalized query ("CERAWeek") in the chat history
            chatHistoryTextArea.innerHTML += "User: " + normalizedQuery + '\n\n';
            chatHistoryTextArea.scrollTop = chatHistoryTextArea.scrollHeight;

            // Send the normalized query ("CERAWeek") to the chat API
            handleUserQuery(normalizedQuery);
